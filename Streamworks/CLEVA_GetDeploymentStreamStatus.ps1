...
$pollingtimeinSecs=2
$Executontimeout = new-timespan -Hours 24
...
$StreamStatus=@{
	...
	{($_ -eq 1) -or ($_ -eq 3)} {
							...
							6   {
									...
									GetStreamJobInfo -CurrentStream $stream -StreamWorksEnvironment $StreamWorksEnvironment
									$DeploymentInfo+="<tr><th>Status</th><td bgcolor='green'>SUCCESS</td></tr></table>"
									$HtmlBody=$HtmlBody -replace "#STREAMINFO#",$DeploymentInfo
									SendMail -To $global:BICLEVADeploymentMail -subject ($Subject + "- Successful") -body $HtmlBody
									Exit 0
								}
							5   {
