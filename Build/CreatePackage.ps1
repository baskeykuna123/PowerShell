...
param([String]$TfsSourceFolder,[String]$TfsStagingFolder,[String]$BuildID,$SearchPattern="*.config.deployment,*.xml.deployment,*.ps1.deployment,*.json.deployment",$dotnetCoreSolution=$false)

if (!$TfsSourceFolder){
	$TfsSourceFolder = "E:\TFSBuild\81\s"
	$TfsStagingFolder = "E:\TFSBuild\81\a"
	$BuildID="Staging_NINA_20201116.5"
	$VerbosePreference = "Continue"
	$dotnetCoreSolution=$false
...
if($dotnetCoreSolution){
    ...
    Get-ChildItem $TfsStagingFolder  | where { $_.PSIsContainer} | foreach {
        $currentstagingFolder = Join-Path $TfsStagingFolder $_.Name
	    Write-Host "Publishing Project :"  $_.Name
        $currentSourceFolder = Get-ChildItem  $TfsSourceFolder -filter $_.Name  -Force -Recurse
        $publishfolder=(Get-ChildItem  $currentSourceFolder.FullName -filter "Publish"  -Force -Recurse).FullName
