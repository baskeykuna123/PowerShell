...
$command=[string]::Format("testrunner.bat `"{1}`" -s`"{2} TestSuite NEW`" -j -r -a",$SOAPUIbinPath,$TestScriptSourcePath,$Environment)
...
$data.testsuite | foreach {
	...
	$TestResultsHTML+=[string]::Format("<TR><TD><B>{0}</B></TD><TD>{1}</TD><TD bgcolor='$bgcolor'>{2}</TD><TD>{3}</TD></TR>",$($_.name),$($_.tests),$($_.failures),$($_.time))
}
...
$EnvironmentStatusHTM = [string]::Format("{0}\{1}_MFCheck_Mainframe_{2}.htm",$global:EnvironmentHTMLReportLocation,$Environment,$status)
$MainframeCheckHTMFile = [string]::Format("{0}\{1}_Mainframe_Check.htm",$global:EnvironmentHTMLReportLocation,$Environment)
...
$HtmlBodyStatus | Out-File Filesystem::$MainframeCheckHTMFile -Force
