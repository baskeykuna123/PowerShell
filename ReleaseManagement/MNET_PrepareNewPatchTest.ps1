Param([String]$Release,[String]$patchNumber,[string]$Assmeblies,[String]$tfsbranch,$ApplicationName,$Startstop,$ClearSoss,[string]$Services,$RecycleIIS)
...
if($patch -ne $null){
	...
	ForEach($service in $Services.Split("`n")){
		if(($ReadPatchManifest.SelectSingleNode("//Patch/Service[@Name='$service']")) -eq $null){
			$Createserviceelement=$ReadPatchManifest.CreateElement("Service")
			$Createserviceelement.SetAttribute("Name", $service)
			$CreateDeploymentActionElement=$ReadPatchManifest.CreateElement("DeploymentActions")
			#$CreateDeploymentActionElement.SetAttribute("Services","$service")	
			$patch.AppendChild($Createserviceelement) | Out-Null
	}
	}
	$CreateDeploymentActionElement.SetAttribute("RecycleIIS","$RecycleIIS")
