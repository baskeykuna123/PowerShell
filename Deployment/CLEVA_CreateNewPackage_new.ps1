...
if(!$DeploymentVersions){
	...
	Write-Host "INFO : Latest Version on $($Environment) : " $currentVersion
	if($PreEnv){
...
#		}
		...
		if($currentVersion -ieq $prevenv){
			Write-Host "`r`nERROR:There are no new versions to be packaged for $($Environment). Error in packaging"
			Exit 1
		}
	}
	else {
	...
	if(!$preversions){
		Write-Host "`r`nERROR :No versions to deploy . Aborting "
		Exit 1
	}

	$DeploymentVersions=@()
	$CurrentRelease=$currentVersion.Split('.')[0]
	Write-host "INFO : Packaging the following new Versions"
...
$currentReleasefolder=Join-Path $Global:ClevaSourcePackages -ChildPath "$Release\"
$latestVersionFolder=Join-Path $Global:ClevaSourcePackages -ChildPath "$Release\$NewVersion"
$UploadTemplatefolder=join-path $Global:ClevaSourcePackages -ChildPath "Templates\UploadTemplate\"
...
foreach($version in $DeploymentVersions){
	...
	if($select){
		$ClevaDBpackage=[String]::Format("{0}{1}\database\cleva\CLEVA_{1}.zip",$currentReleasefolder,$select.Cleva_Version)
