PARAM($Environment,$version,$clientJREVersion="1.6")
...
$ClientsourceBasepath=join-path $Global:ClevaSourcePackages -ChildPath "$Release\$version\$clientsourcefolder"
New-PSdrive  -Name B -Root Filesystem::$ClientsourceBasepath -PSProvider "Filesystem" -Persist | Out-Null
$ClientsourceBasepath="B:\"
$ClientTemplatebasepath=Join-Path $Global:ClevaSourcePackages -ChildPath "templates\ClientTemplates"
...
{		Write-Host "=======================================$($Environment) - $($type)============================="
		...
		New-PSDrive  -Name Y -Root $CitrixTransferlocation -PSProvider "Filesystem" -Persist | Out-Null
		$CurrentEnvClientTemplate=Join-Path $ClientTemplatebasepath -ChildPath "client$($type)\$($Environment)\"
		if($type){$type="-"+$type}
		$currentclient=[string]::Format("Y:\{0}-Current{1}\",$Environment,$type)
		$oldClientName=[string]::Format("{0}-old{1}",$Environment,$type)
		...
		Remove-Item $oldClient -Force -Recurse -ErrorAction Ignore
		...
		if($type -ilike "*TOSCA*"){
			...
			Copy-Item -Path filesystem::"$($ClientTemplatebasepath)\plugins$Type\*" -Destination "$($currentclient)\plugins\" -Force -Recurse	
		}
